...
***************************************************************************/
#ifndef __CPU_ICELAKE_H
#define __CPU_ICELAKE_H
...
#define FOREACH_PMC_HW_EVENT(ACTION)                                                                       \
  ...
  ACTION("MEM_LOAD_L3_HIT_RETIRED:XSNP_NONE", NULL, 0x5308d2)                                        \
  ACTION("MEM_LOAD_L3_MISS_RETIRED:REMOTE_DRAM", NULL, 0x5302d3)                                     \
  ACTION("MEM_LOAD_L3_MISS_RETIRED:LOCAL_DRAM", NULL, 0x5301d3)
...
DECLARE_ENABLE_PMC(icelake, ldm_stall_cycles)
{
...
DECLARE_CLEAR_PMC(icelake, ldm_stall_cycles)
{
}

DECLARE_READ_PMC(icelake, ldm_stall_cycles)
{
   uint64_t l2_MISS_diff  = READ_MY_HW_EVENT_DIFF(0);
   ...
   DBG_LOG(DEBUG, "read stall L2 cycles diff %lu; llc_hit %lu; cycles diff remote_dram %lu; local_dram %lu\n",
		   l2_MISS_diff, llc_hit_diff, remote_dram_diff, local_dram_diff);
   ...
   if (den == 0) return 0;
   return (uint64_t) ((double)l2_MISS_diff * (num / den));
}


DECLARE_ENABLE_PMC(icelake, remote_dram)
{
    ASSIGN_PMC_HW_EVENT_TO_ME("CYCLE_ACTIVITY:STALLS_L2_MISS", 0);
    ASSIGN_PMC_HW_EVENT_TO_ME("MEM_LOAD_L3_HIT_RETIRED:XSNP_NONE", 1);
    ASSIGN_PMC_HW_EVENT_TO_ME("MEM_LOAD_L3_MISS_RETIRED:REMOTE_DRAM", 2);
...
DECLARE_CLEAR_PMC(icelake, remote_dram)
{
}

DECLARE_READ_PMC(icelake, remote_dram)
{
   uint64_t l2_MISS_diff  = READ_MY_HW_EVENT_DIFF(0);
   uint64_t llc_hit_diff     = READ_MY_HW_EVENT_DIFF(1);
   uint64_t remote_dram_diff = READ_MY_HW_EVENT_DIFF(2);
   ...
   double den = num + llc_hit_diff;
   if (den == 0) return 0;
   double stalls = (double)l2_MISS_diff * (num / den);
...
PMC_EVENTS(icelake, 4)
#endif /* __CPU_ICELAKE_H */
